...
`debug-dealii` preset. If deal.II is installed elsewhere, record its CMake
package directory in the machine-local build configuration:

```bash
./build.sh init-config
# Edit build.local.conf and set NMOPT_DEAL_II_DIR to the directory containing deal.IIConfig.cmake.
./build.sh configure debug-dealii
```

The direct CMake equivalent is still available when a one-off configure
override is more convenient:
...
Run the commands from the repository root. The preferred workflow is the
root-level `build.sh` helper. Initialize its machine-local configuration once
per checkout before using `build`, `pipeline`, or `all`:

```bash
./build.sh init-config
```

The backend-neutral profile is the fastest way to verify the repository
without deal.II:
...
Run multiple profiles explicitly or run the complete supported set:
...
```bash
./build.sh pipeline sanitize-neutral
```
...
```bash
./build.sh pipeline release-dealii
```

`build.local.conf` is generated by `init-config` and is ignored by Git. Its
`NMOPT_MAX_JOBS` associative array defines an independent maximum for every
profile, so demanding deal.II profiles can use a lower limit than neutral
profiles. Inspect the effective configuration with:

```bash
./build.sh show-config
```

The helper applies the configured limit to every `build` and `pipeline`
profile. A command-line `--jobs N` value can request fewer jobs, but it cannot
raise the configured maximum. If the configuration is absent, build-producing
commands stop and ask you to run `./build.sh init-config`.

### Manual CMake/CTest equivalent

The helper invokes the checked-in CMake presets directly. For example, the
neutral pipeline corresponds to:
...
The deal.II and sanitizer profiles use the same three commands with
`debug-dealii` or `sanitize-neutral` substituted for the profile name. On a
memory-constrained machine, pass the desired limit explicitly to a manual
deal.II build:
...
Manual commands are useful when a CMake or CTest option needs to be assembled
directly, or when the workflow differs from configure/build/test. They do not
load `build.local.conf`; in particular, manual CMake builds do not inherit
its per-profile job limits unless `--parallel` is supplied explicitly. The
helper's `pipeline` command adds build timing, compact configure output, and
`ctest --progress --no-label-summary`; pass-through options such as
`--cmake-arg`, `--build-arg`, and `--ctest-arg` cover common customizations
without leaving the helper.
