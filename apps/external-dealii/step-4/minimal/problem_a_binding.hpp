  ...
  namespace minimal
  {
    ...
    public:
      ...
      explicit IdentityMetric(LayoutPtr layout)
        : id_("external_step4_minimal_identity")
        , layout_(std::move(layout))
      {
        if (!layout_)
          throw std::invalid_argument(
            "minimal Step-4 identity metric needs a layout");
      }
      ...
      apply(const Primal &primal) const override
      {
        require_compatible(primal.layout(),
                           "minimal Step-4 identity metric primal");
      ...
      inverse_apply(const Covector &covector) const override
      {
        require_compatible(covector.layout(),
                           "minimal Step-4 identity metric covector");
    ...
    public:
      ...
      ProblemABinding(const ProblemABinding &) = delete;
      ProblemABinding &operator=(const ProblemABinding &) = delete;
      ProblemABinding(ProblemABinding &&) = delete;
      ProblemABinding &operator=(ProblemABinding &&) = delete;
    ...
    private:
                   ...
                   const LayoutPtr &test_layout)
      {
        ...
        solvers.solve_adjoint = [problem_ptr, test_layout](
          ...
          const auto result = problem_ptr->solve_adjoint(state_rhs.block(0));
          return SolveResult(
