...
| 2 | Source-oriented triangular $P_1$ state mesh | The source explicitly states triangles and reports counts that constrain the boundary split. First use a cheap structured simplex; add a count-oriented candidate only if topology materially changes the fields. | Structured B2 simplex generation is available; candidate parameter files remain. |
