...
limitation. The investigation now includes an explicit ordinary-normal
transport boundary convention in the B2 semantic policy; the production and
semantic default follows the source, while total conormal remains a diagnostic
override. Corrected
...
run's Armijo trials and final-state native artifacts (`native/mesh-volume.vtu`,
`native/mesh-volume.svg`, `native/fields-volume.vtu`, and
`native/control-boundary.vtu`) for the selected deal.II applications. The
repository-local
