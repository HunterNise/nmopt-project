...
| Energy state tracking | `make_h1_state_tracking_scalar_diffusion_reaction_problem()` | Cellwise volume control with unchanged `l2_cellwise` search metric | The desired state has the selected $`H^{1}_{0}`$ target-data assumption. |
...
| Neumann boundary control | `make_neumann_boundary_control_problem(with_facewise_box, control_discretisation)` | Facewise constants with `l2_facewise` or a continuous degree-one nodal trace with `l2_neumann_trace` | A box is registered only for the facewise representation. |
| Graetz/conservative transport boundary control | `make_neumann_convection_subdomain_tracking_problem(material_id, with_facewise_box, control_discretisation)` | Selected facewise or continuous Neumann trace control | Transport and fixed/inflow/outflow policies must be bound explicitly. |
...
| `scalar-neumann-boundary` | C5.7 boundary tracking/control | Baseline ports; selected Neumann trace control and boundary target | Facewise `l2_facewise` with an optional facewise box, or continuous `l2_neumann_trace` without a box | `reduced_dto` |
| `scalar-neumann-convection-subdomain` | C5.6/Graetz composition | Baseline ports plus `conservative_transport`; a declared fixed-data port when nonzero | Facewise `l2_facewise` with an optional facewise box, or continuous `l2_neumann_trace` without a box | `reduced_dto` |
...
The [Chapter 5 guide](source-catalogue.md) is authoritative
for the mathematical forms, regularity assumptions, source-specific policies,
and registered catalogue variants. Use its [application catalogue](source-catalogue.md#application-catalogue)
