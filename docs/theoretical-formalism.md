...
$$
  ...
  \qquad
  J(x;d)=\sum_{k\in\mathcal K}\Phi_k(O_k(x;d);d). \quad\text{(3)}
$$
...
$$
  -\nabla\cdot(K\nabla y)+b\cdot\nabla y+c y
  = f+\chi_{\omega_c}u\quad\text{in }\Omega, \quad\text{(8)}
$$
...
$$
  y=\widehat y+\ell_D(g_D),\qquad \widehat y\in V. \quad\text{(9)}
$$
...
| State trial space | $`Y=V=\{v \in H^1(\Omega): \mathrm{tr}_{\Gamma_D} v=0\}`$ |
...
\begin{aligned}
 ...
 J&=\tfrac12\|O_\Omega-y_d\|_{L^2(\omega_o)}^2+
       \tfrac\alpha2\|u\|_{L^2(\omega_c)}^2. \quad\text{(12)}
\end{aligned}
$$
...
$$
  \langle E'(\widehat y,u)(w,\delta u),v\rangle
   =a(w,v)-\int_{\omega_c}\delta uv, \quad\text{(13)}
$$
...
$$
  a(w,p)=\int_{\omega_o}(y-y_d)w\quad\text{for every }w\in V. \quad\text{(14)}
$$
...
$$
  j'(u)\delta u=\alpha\int_{\omega_c}u\delta u+
                       \int_{\omega_c}p\delta u. \quad\text{(15)}
$$
...
$$
  B_N(u,v)=\int_{\Gamma_c}u\mathrm{tr}v,
  \qquad U=L^2(\Gamma_c). \quad\text{(16)}
$$
...
$$
  j'(u)\delta u=\alpha(u,\delta u)_{L^2(\Gamma_c)}+
                  (\mathrm{tr}p,\delta u)_{L^2(\Gamma_c)}.
$$
...
$$
  ...
  \qquad \mathrm{tr}_{\Gamma_c}L_Du=u.
$$

With any fixed Dirichlet datum absorbed in $`\ell_0`$, reconstruct

$$
  y=\widehat y+\ell_0+L_Du,\qquad \widehat y\in V. \quad\text{(17)}
$$
...
$$
  j'(u)\delta u=D_uJ\delta u-a(L_D\delta u,p). \quad\text{(18)}
$$
...
$$
  X_q=H(\mathrm{div};\Omega),\quad X_y=L^2(\Omega),
  \qquad Z_1=H(\mathrm{div};\Omega),\quad Z_2=L^2(\Omega).
$$
...
| Boundary tracking | $`O(y)=\mathrm{tr}_{\Gamma_o} y`$, $`Q=L^2(\Gamma_o)`$ or a trace space | available trace and chosen boundary metric |
...
$$
  \tfrac12\|\mathrm{tr}_{\Gamma_o}y-y_{d,\Gamma}\|_{L^2(\Gamma_o)}^2
$$
...
$$
  ...
  =(y(T),v(T))_H-(y(0),v(0))_H-
    \int_0^{T}(y,\dot v)_Hdt. \quad\text{(24)}
$$
...
$$
  \ddot y+A y=f+B u,\qquad y(0)=y_0,\quad\dot y(0)=v_0, \quad\text{(25)}
$$
...
$$
  E_1=\dot y-v,\qquad E_2=\dot v+A y-f-Bu. \quad\text{(26)}
$$
