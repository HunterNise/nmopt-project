...
namespace nmopt::compiler::v1
{
            ...
            const CompilationProduct               product) const
    {
      ...
      if (uses_neumann_boundary_control)
      ...
      else if (uses_dirichlet_control)
        {
          ...
          solvers = make_state_adjoint_solvers(dirichlet,
                                               policy.state_solve,
                                               policy.adjoint_solve);
      ...
      else if (uses_coefficient_identification)
        {
          ...
          solvers = make_state_adjoint_solvers(coefficient,
                                               policy.state_solve,
                                               policy.adjoint_solve);
  ...
  private:
    ...
    template <typename Model>
    static contract::StateAdjointSolversT<dealii_backend::SerialBackend>
    make_state_adjoint_solvers(
      ...
      dealii_backend::SPDLinearSolvePolicy adjoint_solve)
    {
      ...
      contract::require(static_cast<bool>(model),
                        "Compiled services need a typed executable model");
      return contract::StateAdjointSolversT<Backend>{
        [model, state_solve = std::move(state_solve)](
          const Primal &control) {
          return model->solve_state_with_report(control, state_solve);
        },
        [model, adjoint_solve = std::move(adjoint_solve)](
          const Primal &full_point,
          const Covector &state_rhs) {
          return model->solve_adjoint_with_report(full_point,
                                                  state_rhs,
                                                  adjoint_solve);
        }};
    }
      ...
      const semantic::v1::SuppliedOTDDeclaration * supplied_otd_declaration)
    {
      ...
      compatibility.backend = "deal.II serial Vector<double>";
      compatibility.execution = "assembled";
      ...
      compatibility.state_space = uses_l2_dirichlet_control
      ...
      compatibility.control_space =
        control_space_description(target,
                                  ...
                                  uses_continuous_neumann_trace,
                                  registration);
      compatibility.quadrature = volume_quadrature;
      compatibility.dual_representation = "tested dual coefficients with dot pairing";
      compatibility.data_rule = uses_section_5_11_dirichlet_control
      ...
      compatibility.observation_realisation = uses_h1_state_observation
        ? (uses_dirichlet_control_lifting
      ...
      if (uses_h1_control_regularisation)
        compatibility.declared_assumptions.push_back(uses_h1_dirichlet_control
