...
namespace
{
    ...
    const double expected_forcing = 0.0)
  {
    ...
    if (discretisation ==
      ...
      require(manifest.compatibility.control_space.find(
                "homogeneous-Dirichlet scalar FE_Q(1)") != std::string::npos &&
                manifest.compatibility.metric_solve_policy.find("l2_continuous") !=
                  std::string::npos,
              "B1 continuous candidate selected the wrong compiled control");
    else
      require(manifest.compatibility.control_space.find("FE_DGQ(0)") != std::string::npos,
